              ...
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-blue-400 font-bold">
                    {completedStops.length + cancelledStops.length}/{stops.length}
                  </span>
                </div>
              </CardHeader>
              ...
              <Card className="bg-slate-800/50 border-slate-700">
                <CardHeader className="pb-2">
                  <CardTitle className="text-white text-sm flex items-center gap-2">
                    ...
                    Done ({completedStops.length} delivered{cancelledStops.length > 0 ? `, ${cancelledStops.length} cancelled` : ""})
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-2">
                  {[...completedStops, ...cancelledStops].slice(-4).map((stop: any) => (
                    <div
                      key={stop.id}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {stop.status === "cancelled" && <p className="text-red-400 text-xs">Not delivered</p>}
                      </div>
                    </div>
                  ))}
                </CardContent>
              ...
              <Card className="bg-green-500/10 border-green-500/30">
                <CardContent className="py-8 text-center">
                  ...
                  <p className="text-slate-400">
                    {completedStops.length} delivered{cancelledStops.length > 0 ? `, ${cancelledStops.length} couldn't be delivered` : ''}.
                  </p>
                  {route?.endAddress && (
                ...
                <CardContent className="space-y-4">
                  ...
                  {cancelledStops.length > 0 && (
                    <div className="p-3 bg-red-500/10 rounded border border-red-500/20 text-center">
                      <p className="text-xl font-bold text-red-400">{cancelledStops.length} couldn't be delivered</p>
                    </div>
                  )}
                  ...
                  <div className="border-t border-slate-600 pt-4">
                    ...
                    <div className="space-y-3 max-h-64 overflow-y-auto">
                      {completedStops.map((stop: any, idx: number) => (
                        <div key={stop.id} className="p-3 bg-slate-700/30 rounded border border-slate-600">
                          <div className="flex items-start justify-between gap-2">
                            <div className="flex-1">
                              <div className="flex items-center gap-2 mb-1">
                                <span className="bg-green-500/20 text-green-400 text-xs font-bold px-2 py-0.5 rounded">#{idx + 1}</span>
                                <span className="text-xs font-mono text-blue-400">
                                  {stop.delivery?.deliveryIdentifier || `DEL${stop.delivery?.id}`}
                                </span>
                              </div>
                              <p className="text-white text-sm font-medium">{stop.delivery?.customerName || 'Customer'}</p>
                              <p className="text-slate-400 text-xs mt-1">{stop.delivery?.addressText}</p>
                              ...
                              {stop.completedAt && (
                                <p className="text-slate-500 text-xs mt-2">
                                  Delivered: {new Date(stop.completedAt).toLocaleString()}
                                </p>
                              )}
                            </div>
                      ...
                      {completedStops.length === 0 && (
                        <p className="text-slate-500 text-center py-4">No delivered stops yet</p>
                      )}
                    </div>
                  </div>
                    ...
                    <div className="border-t border-slate-600 pt-4">
                      ...
                      <div className="space-y-3 max-h-64 overflow-y-auto">
                        {cancelledStops.map((stop: any) => (
                          <div key={stop.id} className="p-3 bg-red-500/5 rounded border border-red-500/20 opacity-75">
                            <div className="flex items-start justify-between gap-2">
                              <div className="flex-1">
                                <div className="flex items-center gap-2 mb-1">
                                  <span className="bg-red-500/20 text-red-400 text-xs font-bold px-2 py-0.5 rounded">CANCELLED</span>
                                  <span className="text-xs font-mono text-slate-400">
                                    {stop.delivery?.deliveryIdentifier || `DEL${stop.delivery?.id}`}
                                  </span>
                                </div>
                                <p className="text-slate-300 text-sm font-medium">{stop.delivery?.customerName || 'Customer'}</p>
                                <p className="text-slate-400 text-xs mt-1">{stop.delivery?.addressText}</p>
                                {stop.delivery?.prescriptions && stop.delivery.prescriptions.length > 0 && (
                                  <div className="mt-2 flex flex-wrap gap-1">
                                    {stop.delivery.prescriptions.map((rx: any, rxIdx: number) => (
                                      <span key={rxIdx} className="bg-slate-600/50 text-slate-400 text-xs px-2 py-0.5 rounded">
                                        Rx: {rx.rxNumber}
                                      </span>
                                    ))}
                                  </div>
                    ...
                    <div className="border-t border-slate-600 pt-4">
                      ...
                      <div className="space-y-2 max-h-40 overflow-y-auto">
                        {pendingStops.map((stop: any, idx: number) => (
                          <div key={stop.id} className="p-2 bg-slate-700/20 rounded border border-slate-700 flex items-center gap-2">
                            <span className={`text-xs font-bold px-2 py-0.5 rounded ${
                              stop.priority === "urgent" ? "bg-red-500/20 text-red-400" : "bg-yellow-500/20 text-yellow-400"
                            }`}>
                              {stop.priority === "urgent" ? "URGENT" : `#${completedStops.length + cancelledStops.length + idx + 1}`}
                            </span>
                            <div className="flex-1 min-w-0">
                          ...
                          <div key={route.id} className="border border-slate-600 rounded-lg overflow-hidden">
                            ...
                            {route.stops && route.stops.length > 0 && (
                              <div className="p-3 space-y-2 max-h-64 overflow-y-auto bg-slate-800/50">
                                ...
                                {route.stops.filter((s: any) => s.status === 'completed').map((stop: any) => (
                                  <div key={stop.id} className="flex items-start gap-2 p-2 bg-slate-700/30 rounded">
                                    ...
                                    <div className="flex-1 min-w-0">
                                      ...
                                      <p className="text-white text-sm">{stop.delivery?.customerName || 'Customer'}</p>
                                      <p className="text-slate-400 text-xs truncate">{stop.delivery?.addressText}</p>
                                      {stop.completedAt && (
                                        <p className="text-slate-500 text-xs mt-1">{new Date(stop.completedAt).toLocaleString()}</p>
                                      )}
                                    </div>
                                  </div>
                                ))}
                                {/* Cancelled stops — visible but clearly marked, no action buttons */}
                                {route.stops.filter((s: any) => s.status === 'cancelled').map((stop: any) => (
                                  <div key={stop.id} className="flex items-start gap-2 p-2 bg-red-500/5 rounded border border-red-500/15">
                                    ...
                                    <div className="flex-1 min-w-0">
                                      <div className="flex items-center gap-1 mb-0.5">
                                        ...
                                        <span className="text-xs text-red-400 font-medium">· Not delivered</span>
                                      </div>
                                      <p className="text-slate-300 text-sm">{stop.delivery?.customerName || 'Customer'}</p>
                                      <p className="text-slate-500 text-xs truncate">{stop.delivery?.addressText}</p>
                                      {stop.notes && (
                                        <p className="text-red-400/60 text-xs mt-0.5 italic">{stop.notes.replace(/^CANCELLED:\s*/i, '')}</p>
                                      )}
                                    </div>
                                  </div>
                                ))}
                                {route.stops.filter((s: any) => s.status === 'completed' || s.status === 'cancelled').length === 0 && (
                                  <p className="text-slate-500 text-xs text-center py-2">No stops recorded</p>
                                )}
                              </div>
                            )}
                          </div>
